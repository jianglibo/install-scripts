...
@rem for /f %%a IN ('dir /b hive-beeline-**.jar') do (
@rem   set HADOOP_CLASSPATH=%HADOOP_CLASSPATH%;%HIVE_HOME%\lib\%%a
@rem )
@rem for /f %%a IN ('dir /b super-csv-**.jar') do (
@rem   set HADOOP_CLASSPATH=%HADOOP_CLASSPATH%;%HIVE_HOME%\lib\%%a
@rem )
@rem for /f %%a IN ('dir /b jline-**.jar') do (
@rem set HADOOP_CLASSPATH=%HADOOP_CLASSPATH%;%HIVE_HOME%\lib\%%a
@rem )
@rem for /f %%a IN ('dir /b hive-jdbc-**-standalone.jar') do (
@rem   set HADOOP_CLASSPATH=%HADOOP_CLASSPATH%;%HIVE_HOME%\lib\%%a
@rem )
...
@echo "%JAVA_HOME%\bin\java %JAVA_HEAP_MAX% %HADOOP_OPTS% -classpath %CLASSPATH% org.apache.hive.beeline.BeeLine %*"
